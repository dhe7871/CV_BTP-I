...
%%UAV Specifications
va = 25; %m/s
L = [10, 25, 50, 75, 100]; %m (should always be greater than r; if L > r, then the algorithm can be dynamically unstable i.e. oscillation never dies)
...
parfor e = 1:Ne
    ...
    for t=dt:dt:simTime
        ...
        s = getIntersectionPt(o, r, p, L(e), direction);
        ...
        dpsi = max(min(2*va*sin(eta)/L(e), dpsiMax), -dpsiMax);
...
xlabel("x [m]");
ylabel("y [m]");
title("Non-Linear Guidance Law for Loiter (Variation L)");
legend([h1, h2, h], leg);
hold off;
